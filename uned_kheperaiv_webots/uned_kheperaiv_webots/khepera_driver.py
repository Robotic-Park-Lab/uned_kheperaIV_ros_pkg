...
class KheperaWebotsDriver:
    ...
    def initialize(self):
        self.node.get_logger().info('Connected to Webots -> Khepera %s' % (str(self.id)))
        ...
        self.range_publisher = self.node.create_publisher(Range, self.id+'/range0', 10)
        ...
        if self.config['controller']['enable']:
            ...
            if self.config['controller']['type'] == 'ipc':
                ...
                self.eomas = 3.14
                self.node.get_logger().info('%s::IPC Controller' % (str(self.id)))
            else:
                self.position_controller_IPC = False
                self.node.get_logger().info('%s::Force Field Controller' % (str(self.id)))
        else:
            self.position_controller = False
            self.node.get_logger().info('%s::Open Loop' % (str(self.id)))
    ...
    def publish_laserscan_data(self):
        ...
        self.msg_laser.header.frame_id = self.id+'/base_link'
    ...
    def load_formation_params(self):
        ...
        if task_type == 'distance':
            ...
            for rel in self.relationship:
                ...
                if not id.find("line") == -1:
                ...
                elif aux[0] == 'origin' or aux[0] == 'sphere':
                    auxa = aux[1]
                    auxb = auxa.split('-')
                    origin = Point()
                    ...
                    self.node.get_logger().debug('Agent: %s: Cono: a %s \tc: %s' % (self.id, auxb[0], auxb[1]))
                    robot = Agent(self, self.node, aux[0], d = float(auxb[0]),point = origin)
    ...
    def distance_gradient_controller(self):
        if self.formation:
            ...
            if '_v' in self.controller_type:
                ...
                self.target_pose.pose.position.z = self.pose.position.z
                self.target_twist.linear.x = 2*dx
                self.target_twist.linear.y = 2*dy
